...
impl WorkspaceState {
    ...
    fn to_ui_data(&self) -> Vec<WorkspaceButtonData> {
        self.workspaces
            .iter()
            .map(|(id, name, focused, idx)| WorkspaceButtonData {
                target: WorkspaceTarget::NiriId(*id),
                name: name.clone(),
                focused: *focused,
                sort_key: *idx as i64,
            })
...
pub(super) fn connect_socket() -> Result<UnixStream, String> {
...
pub(super) fn run(
    ...
    std::thread::spawn(move || {
        while let Some(target) = rx_ws.blocking_recv() {
            let WorkspaceTarget::NiriId(ws_id) = target else {
                continue;
            };
    ...
    match event_result {
        Ok(resp) => eprintln!("[workspaces] niri EventStream response: {:?}", resp),
        Err(e) => {
            eprintln!("[workspaces] niri EventStream failed: {}", e);
            return;
        }
    }
    ...
    loop {
        ...
        let event: serde_json::Value = match serde_json::from_str(&line) {
            Ok(e) => e,
            Err(e) => {
                eprintln!("[workspaces] niri parse error: {}", e);
                continue;
            }
        };
